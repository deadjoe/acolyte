    ...
    async def process_content(self, content: str, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.get("success", False):
                ...
                logger.error(f"Ollama处理失败: 耗时={elapsed_time:.2f}秒, 错误={response.get('error')}")
    ...
    async def _process_with_api(self, system_prompt: str, user_prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                logger.debug(f"Ollama请求完成: 状态码={response.status_code}, 耗时={request_time:.2f}秒")
                ...
                if "response" in response_json:
                    ...
                    logger.debug(f"Ollama响应解析完成: 解析耗时={parse_time:.2f}秒, 总耗时={(time.time()-start_time):.2f}秒")
            ...
            logger.error(f"Ollama HTTP错误: 状态码={e.response.status_code}, URL={e.request.url}, 耗时={(time.time()-start_time):.2f}秒")
    ...
    async def _test_connection(self) -> Dict[str, Union[bool, str]]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                request_time = time.time() - start_time
                logger.debug(f"Ollama连接测试响应: 状态码={response.status_code}, 耗时={request_time:.2f}秒")
                ...
                if "models" in models_response:
                    ...
                    if self.model_name in model_names:
                        logger.info(f"Ollama连接测试成功: 找到模型={self.model_name}, 耗时={time.time()-start_time:.2f}秒")
                        return {
                            "success": True, 
                        ...
                        logger.warning(f"Ollama连接测试部分成功: 未找到模型={self.model_name}, 可用模型={available_models}, 耗时={time.time()-start_time:.2f}秒")
                        return {
                            "success": False, 
            ...
            logger.error(f"Ollama连接测试HTTP错误: 状态码={e.response.status_code}, 耗时={elapsed_time:.2f}秒, 错误={error_details}")
            return {
                "success": False, 
