...
class ReviewProcessor(BaseTaskProcessor):
    ...
    def _create_review_prompt(self, results: List[Dict]) -> str:
        ...
        review_prompt += (
            ...
            "3. 对每个LLM的分析进行评估，并选出最佳的分析结果\n"
            "4. 给出你的评估报告，包含以下部分：\n"
            "   - 各结果评估：对每个LLM的分析结果进行评估\n"
            "   - 比较分析：比较不同LLM的分析方法和结果\n"
            "   - 最佳结果：选出最佳的分析结果并说明理由\n"
            "   - 综合建议：给出改进分析方法的建议\n\n"
            "## 输出格式\n\n"
            "请使用Markdown格式输出你的评估报告。为了保持与原始分析结果的兼容性，请在报告中包含以下量化评分：\n"
            "```json\n"
            "{\n"
            '  "偏见指数": { "名称": "BI", "分数": 0.0 },\n'
            '  "误导性指数": { "名称": "MI", "分数": 0.0 },\n'
            ...
            "}\n"
            "```\n"
            "请将上述JSON中的分数替换为你认为最准确的LLM的分数。这些分数应该来自你选出的最佳分析结果，而不是你自己对原始文章的分析。"
        )
