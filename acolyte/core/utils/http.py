    ...
    def request(
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                logger.debug(f"HTTP响应: {method} {log_url} - 状态码: {response.status_code}, 耗时: {elapsed_time:.2f}秒")
    ...
    async def arequest(
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                elapsed_time = time.time() - start_time
                logger.debug(f"异步HTTP响应: {method} {log_url} - 状态码: {response.status_code}, 耗时: {elapsed_time:.2f}秒")
