    ...
    with patch.dict("sys.modules", {"acolyte.cli.commands": MagicMock()}):
        with patch("acolyte.cli.history_show.AcolyteClient", return_value=mock_client):
            ...
            with (
                ...
                # 调用被测试的函数，指定LLM ID为2
                await show_task(
                    task_id=1, all_results=False, specified_llm_id=2, raw=False, format_type="table"
                )
...
class TestShowFunctions:
    ...
    async def test_show_all_llm_results(self):
        ...
        mock_client.get_task_results = AsyncMock(
            return_value=[{"llm_id": 1, "bias_index": 7.5}, {"llm_id": 2, "bias_index": 8.5}]
        )
        mock_client.get_llms = AsyncMock(
            return_value=[{"id": 1, "name": "LLM1"}, {"id": 2, "name": "LLM2"}]
        )
