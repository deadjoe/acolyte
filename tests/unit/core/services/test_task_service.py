...
class TestTaskService:
    ...
    def mock_task_class(self):
        ...
        class MockTask:
            ...
            def to_dict(self, include_content=True):
                ...
                result = {
                    "id": self.id,
                    "processing_mode": self.processing_mode.value if hasattr(self.processing_mode, "value") else self.processing_mode,
    ...
    def service(self):
        ...
        with patch("acolyte.core.services.task_service.TaskProcessor") as mock_processor_class:
            ...
            # 模拟PromptManager
            with patch("acolyte.core.services.task_service.PromptManager") as mock_prompt_manager_class:
    ...
    async def test_create_task_in_db_success(self, service, mock_task_class):
        ...
        with patch("acolyte.core.services.task_service.run_in_session", side_effect=mock_run_in_session):
            # 执行测试
    ...
    async def test_internal_get_task_success(self, service):
        ...
        async def mock_run_in_session(callback):
            ...
            mock_result1.to_dict = MagicMock(return_value={"id": 1, "task_id": task_id, "llm_id": 1, "content": "结果1", "bias_index": 0.1})
            ...
            mock_result2.to_dict = MagicMock(return_value={"id": 2, "task_id": task_id, "llm_id": 2, "content": "结果2", "misleading_index": 0.2})
            
            result_query_mock = MagicMock()
            result_filter_mock = MagicMock()
            result_filter_mock.all = MagicMock(return_value=[
        ...
        with patch("acolyte.core.services.task_service.run_in_session", side_effect=mock_run_in_session):
            with patch("acolyte.core.services.task_service.extract_model_data", return_value={"id": task_id, "content": "测试内容", "status": "completed"}):
                # 执行测试
