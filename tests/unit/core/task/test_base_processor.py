...
class TestBaseTaskProcessor:
    ...
    async def test_get_task_data(self, processor, mock_session_run):
        ...
        with patch('acolyte.core.task.processors.base.extract_model_data') as mock_extract:
            ...
            mock_extract.assert_called_once_with(mock_task, include_relationships=False)
    ...
    async def test_save_result(self, processor, mock_session_run):
        ...
        with patch('acolyte.core.task.processors.base.TaskResult') as MockTaskResult:
            ...
            mock_session.query.assert_called_once()
            mock_query.filter_by.assert_called_once_with(id=task_id)
            
            # 验证TaskResult创建
            MockTaskResult.assert_called_once()
            call_kwargs = MockTaskResult.call_args.kwargs
            assert call_kwargs["task_id"] == task_id
            assert call_kwargs["llm_id"] == llm_id
            assert call_kwargs["raw_response"] == result["raw_response"]
            assert call_kwargs["processed_result"] == result["processed_result"]
            assert call_kwargs["bias_index"] == result["bias_index"]
            assert call_kwargs["misleading_index"] == result["misleading_index"]
            assert call_kwargs["hidden_intent_index"] == result["hidden_intent_index"]
            assert call_kwargs["credibility_score"] == result["credibility_score"]
            
            # 验证任务最终结果更新
            assert mock_task.final_result_id == mock_task_result.id
            assert isinstance(mock_task.updated_at, datetime)
