...
class TestPromptManager:
    ...
    @patch('acolyte.core.prompt.manager.db')
    def test_get_prompt_by_version_with_model_target(self, mock_db, prompt_manager, mock_prompt_class):
        """测试带模型目标的版本查询"""
        # 准备模拟数据
        mock_session = mock_db.session_scope.return_value.__enter__.return_value
    ...
    @patch('acolyte.core.prompt.manager.db')
    def test_get_latest_prompt_model_specific_not_found(self, mock_db, prompt_manager, mock_prompt_class):
        """测试指定模型目标但未找到时的行为"""
        # 准备模拟数据
