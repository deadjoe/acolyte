"""
Anthropic Claude LLM客户端单元测试

测试AnthropicClient类的功能和行为。
"""
...
class TestAnthropicClient:
    """AnthropicClient类的测试用例"""

    @pytest.fixture
    def llm_config(self):
        """创建测试用的LLM配置"""
        config = MagicMock(spec=LlmConfig)
        config.name = "Test Claude"
    ...
    def test_init_with_default_base_url(self):
        ...
        with patch("acolyte.core.llm.base.LlmClient._normalize_base_url") as mock_normalize:
            mock_normalize.return_value = "https://api.anthropic.com/v1"
    ...
    def test_init_with_invalid_api_key(self, caplog):
        """测试初始化时API密钥格式不正确"""
        # 创建配置，API密钥格式不正确
        config = MagicMock(spec=LlmConfig)
        config.name = "Test Claude"
    ...
    @pytest.mark.asyncio
    @patch("acolyte.core.llm.providers.anthropic.AnthropicClient._make_request")
